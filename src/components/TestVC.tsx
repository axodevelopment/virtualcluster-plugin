...
export default function ExamplePage() {
  ...
  //const namespace = 'operator-virtualcluster'; // Replace with your namespace

  useEffect(() => {
    setVirtualClusters(fakeData);
  }, []);
  /*
  found this online need to lookup k8sGet hwo it gets context etc etc
  useEffect(() => {
    const fetchVirtualClusters = async () => {
      try {
        ...
        setVirtualClusters(response.);
      } catch (error) {
        console.error('Error fetching VirtualClusters:', error);
      }
    };
